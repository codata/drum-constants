...
def parse_workbook(filename):
    ...
    sheet_constants_instances = get_sheet_entries(wb['ConstantsInstances'], ["id","name","nist_id","nist_deprecated_ids","units_si","units_ucum","units_uom","constant_id","qudt_id"])
    ...
    for (id, entry) in sheet_constants_instances.items():
        ...
        if constant:
            ...
            constant_instance_ids = {'CODATA': codata_id, 'NIST': entry.get('nist_id')}
